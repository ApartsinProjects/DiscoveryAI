export const meta = {
  name: "pack-p4b-prose-add",
  description: "Pack P4b: Prose additions — engagement, aha-moments, memorability (#16 #24 #27)",
  phases: [{ title: "Prose Add" }],
}
...
await pipeline(SECTIONS, async (t) => {
  return agent(
    `You are adding content to prose in section ${t.sec} — "${t.title}".
...
=== PASS 1: Engagement / Why Should I Care (Agent #16) ===
Find the paragraph that introduces the section's core technique or algorithm. Prepend
a 1-2 sentence "why this matters" bridge: a real consequence, failure mode, or
application that makes the reader feel the importance BEFORE the mechanism is explained.
Mark data-agent="16". Skip if such a bridge already exists.

=== PASS 2: Aha-Moment (Agent #24) ===
Find one non-obvious comparison or striking number buried in the section. Elevate it:
insert <span class="aha-moment">...</span> around the striking fact, or add a
parenthetical aside in bold: <strong>(This is why X outperforms Y by 10x on Z.)</strong>
One insertion maximum. Mark data-agent="24". Skip if an aha-moment span already exists.

=== PASS 3: Memorability (Agent #27) ===
Bold the 2-3 most important technical phrases in the section body (not in headings, not
in callouts) using <strong>phrase</strong>. Then, at the end of the last paragraph
before the first callout or code block, append ONE memorable one-liner: a single
sentence crystallizing the section's insight. Prefix it: <em>In short:</em> Mark
data-agent="27". Skip bolding if 3+ <strong> tags already exist in prose.

Write the complete updated file with all edits applied.
`,
    { label: `pack-p4b-prose-add-${t.sec}`, phase: "Prose Add" }
  )
})

return { wave: "pack-p4b-prose-add", sections: SECTIONS.length }
