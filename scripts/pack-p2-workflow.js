export const meta = {
  name: "pack-p2-integrity",
  description: "Pack P2: Integrity — fact-check, terminology consistency, content currency (#11 #12 #20)",
  phases: [{ title: "Integrity" }],
}
...
await pipeline(SECTIONS, async (t) => {
  return agent(
    `You are running integrity passes on section ${t.sec} — "${t.title}".
...
=== PASS 1: Fact Integrity (Agent #11) ===
Scan every factual claim: benchmark numbers, accuracy figures, publication years,
model names, dataset sizes, algorithmic complexity claims. For each claim that is
wrong, outdated (pre-2022), or unattributed, correct it in-place. Add "(circa YYYY)"
if the figure is time-sensitive. Do NOT add footnote citations — edit the claim text
directly. Mark corrected spans data-agent="11". If all claims are correct, make no
prose changes.

=== PASS 2: Terminology Consistency (Agent #12) ===
Apply throughout the file:
- First use of every acronym: "full phrase (ACRONYM)". Subsequent uses: acronym only.
- Replace "In this section/lecture/module we will" -> delete or restate as assertion.
- Replace "students" -> "readers", "course" -> "book", "lecture" -> "section".
- Remove em dashes and double hyphens: replace with comma, semicolon, colon, or parens.
Mark changed spans data-agent="12".

=== PASS 3: Content Currency (Agent #20) ===
Identify any claim that references a specific tool, library version, or benchmark that
has been superseded since 2022. For each: add a parenthetical update inline
("as of 2024, [newer tool/result] has largely replaced this") or correct the claim.
Do NOT delete the original explanation — extend it. Mark additions data-agent="20".
If no outdated content is found, make no changes.

Write the complete updated file with all edits applied.
`,
    { label: `pack-p2-integrity-${t.sec}`, phase: "Integrity" }
  )
})

return { wave: "pack-p2-integrity", sections: SECTIONS.length }
