export const meta = {
  name: "pack-p1a-depth",
  description: "Pack P1a: Core depth — what/why/how/when, analogies, warnings, research frontier, projects (#02 #06 #10 #18 #23)",
  phases: [{ title: "Depth" }],
}
...
await pipeline(SECTIONS, async (t) => {
  return agent(
    `You are improving section ${t.sec} — "${t.title}".
File path: ${BOOK_ROOT}/${t.path}

Read the file once. Perform ALL five passes below in order. Write the file ONCE after
all passes. Never write intermediate versions. Do not use em dashes or double hyphens.
Do not use "honestly", "frankly", "candidly", "to be honest", or "in truth".

=== PASS 1: Deep Explanation (Agent #02) ===
Find the single most surface-level concept. Insert a 2-3 sentence paragraph immediately
after its first <p> covering: WHAT it is precisely, WHY it matters, HOW it works
(one-sentence mechanism), WHEN to use it vs. alternatives. Mark data-agent="02". Skip
if already present.

=== PASS 2: Analogy / Mental Model (Agent #06) ===
Find the most abstract or counterintuitive concept. Add <div class="callout tip">
<h4>Mental Model</h4><p>...</p></div> with a concrete everyday analogy (NOT a robotics
analogy). The analogy must map mechanism, not just label. Mark data-agent="06". Skip
if an analogy callout already exists for this concept.

=== PASS 3: Misconception Warning (Agent #10) ===
Identify the most common student misconception about the main topic. Add
<div class="callout warning"><h4>Common Misconception</h4><p>...</p></div> after the
paragraph that could most easily be misread. Name the misconception explicitly and
correct it in one sentence. Mark data-agent="10". Skip if a warning callout already
addresses it.

=== PASS 4: Research Frontier (Agent #18) ===
Add or update ONE <div class="callout research-frontier"><h4>Research Frontier</h4>
<p>...</p></div> citing a 2023+ development (paper, benchmark, system, or technique)
that pushes beyond what the section teaches. Name the paper or system. If a
research-frontier callout predates 2023, update it. Mark data-agent="18".

=== PASS 5: Hands-On Project (Agent #23) ===
Add ONE <div class="callout practical-example"><h4>Try It: [short title]</h4>
<p>...</p></div> with a specific 3-5 step mini-project a reader could complete with a
laptop and standard Python libraries. Steps must be concrete. Place it near the end of
the section. Mark data-agent="23". Skip if a practical-example callout already has
numbered steps.

Write the complete updated file with all edits applied.
`,
    { label: `pack-p1a-depth-${t.sec}`, phase: "Depth" }
  )
})

return { wave: "pack-p1a-depth", sections: SECTIONS.length }
